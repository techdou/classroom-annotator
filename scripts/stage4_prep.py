...
OP_RULES = """## 操作类型速查（11 类，金标准校准版）
书写WR: 黑板/白板/PPT批注区/纸质材料上新增文字、符号、图形（新增！擦除已有内容是擦改EC）
擦改EC: 对已有内容擦除/删除/覆盖/订正
展示DP: 学生持续观察某材料成为主要活动时（指示看图后学生看图作答）。教师讲授中短暂说"看一下/翻到X页"不算展示，算讲述TL（R3）
指向PT: 手/教鞭/激光笔有明确方向性地指向具体位置（纯视觉特征，依赖视觉事件，ASR 无信号）
播放PL: 启动/暂停/继续/停止视频、音频、自动动画
使用OP: 教师操作设备且不说话的纯操作段（通常仅数秒，如按计时器）。边操作边讲话 → 讲述TL（R2）
演示DE: 实际操作实物/教具/实验器材/动态软件呈现过程或变化
巡视PA: 走入学生座位区走动、停留、低头查看；巡视期间教师简短提示语并入巡视段（R1）
观察OB: 原地静止地看/听学生回答/展示/讨论（走动查看是巡视；听完后评价是讲述）
讲述TL: 口头朗读/说明/解释/归纳/评价；连续追问、反问、自问自答、快速核对（"同意不"）都算讲述（R4）——拿不准向讲述收敛
提问QN: 仅用于明确点名/邀请学生作答且学生实际作答的独立段（"来，你说一下"）（R4）
...
## 切分粒度（按金标准校准）
中位10-15秒，短可2-3秒，静默做题/讨论段可超1分钟；讲述长段（>40s）按内容主题切换拆分。
金标准用量参考（47.8分钟课109条）：讲述60+/巡视20/指向8/观察4/书写4/使用4/提问3/展示2/播放1。
若你的提问或展示远超该量级，说明 R3/R4 判定过松，收紧后再输出。每条操作只标一个主要操作类型。
